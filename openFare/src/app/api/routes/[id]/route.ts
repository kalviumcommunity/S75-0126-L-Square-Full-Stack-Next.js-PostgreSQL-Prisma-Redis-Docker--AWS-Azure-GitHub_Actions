...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (isNaN(id)) return NextResponse.json({ success: false, error: 'Invalid route ID' }, { status: 400 });
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!existingRoute) return NextResponse.json({ success: false, error: 'Route not found' }, { status: 404 });
    ...
    return NextResponse.json({ success: true, data: updatedRoute, message: 'Route updated successfully' });
  } catch (error) {
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (existingRoute._count.schedules > 0)
      return NextResponse.json({ success: false, error: 'Cannot delete route with existing schedules' }, { status: 409 });
